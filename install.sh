...
if command -v sddm-greeter-qt6 >/dev/null 2>&1; then
    ...
    # fallback: check Qt version via sddm binary
    if sddm --version 2>/dev/null | grep -q "Qt 6"; then
        SYSTEM_QT="6"
        GREETER_CMD="sddm-greeter-qt6"
        echo -e "${BLUE}==>${NC} System detected: ${GREEN}Qt6 (Modern)${NC}"
    else
        SYSTEM_QT="5"
        GREETER_CMD="sddm-greeter"
        echo -e "${BLUE}==>${NC} System detected: ${YELLOW}Qt5 (Legacy)${NC} (fallback)"
    fi
fi
...
if [ "$SYSTEM_QT" = "6" ]; then
    ...
    if ! find /usr/lib /usr/lib64 2>/dev/null | grep -q "qtvirtualkeyboard"; then
        MISSING+=("qt5-virtualkeyboard (or libqt5virtualkeyboard5)")
    fi
fi
...
echo ""
echo -e "${GREEN}Done!${NC} Reboot to see Hypr SDDM on your login screen."#!/bin/bash
set -e
